...
class DeudaOperativaScreenOrder extends StatefulWidget {
  const DeudaOperativaScreenOrder({Key? key}) : super(key: key);

  @override
  State<DeudaOperativaScreenOrder> createState() =>
      _DeudaOperativaScreenOrderState();
}
          ...
          return BlocBuilder<MainBloc, MainState>(
            builder: (context, state) {
              List<DeudaOperativaSingle>? data =
                  state.deudaOperativa?.deudaOperativaListSearch2;
              if (data == null)
                return Center(child: CircularProgressIndicator());
              return tableData(
                datos: data,
