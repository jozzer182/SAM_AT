...
import 'package:samat2co/resources/file_uploader.dart';
...
class FieldPre extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (tipoCampo == TipoCampo.fecha) {
      controller.text =
          DateTime.parse(initialValue).toString().substring(0, 10);
    }
    if (flex == null) {
      return SizedBox(
        height: 40,
...
class FieldDate extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    dateController.text = DateTime.parse(date).toString().substring(0, 10);
    if (flex == null) {
      return SizedBox(
        height: 40,
        child: _intField(context),
